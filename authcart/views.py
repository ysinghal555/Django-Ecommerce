...
def signup(request):
    if request.method == "POST":
        ...
        message = render_to_string('activate.html', {
            ...
            # Here we are encoding the uid details
            'uid': urlsafe_base64_encode(force_bytes(user.pk)),
            'token': generate_token.make_token(user)

        })

        #  To send the email, but to get the mail on google we need business email address
...
class ActivateAccountView(View):
    def get(self, request, uidb64, token):
        try:
            # Here we are decoding the uid details
            uid = force_str(urlsafe_base64_decode(uidb64))
            user = User.objects.get(pk=uid)
        except Exception as identifier:
            user = None

        # It is checking if the generated token is same and will only make the use isActive
